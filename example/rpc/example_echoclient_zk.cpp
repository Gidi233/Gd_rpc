...
#include "../gdrpc/rpc/zkClient.hpp"
...
int main(int argc, char* argv[]) {
  ...
  ZkClientPtr zkCli;
  zkCli->Start();
  string path = "/EchoService";
  RpcCluster<EchoRpcClient, RoundRobin<Client<EchoRpcClient>>> cluster(
      loopThread.startLoop(), zkCli, path);
  ...
  auto echo = [](const string& s) {
    return [s](EchoRpcClient* obj) {
      return invoke(&EchoRpcClient::echo, *obj, s);
    };
  };
  for (int i = 0; i < 5; i++) {
